...
埋め込み `X` は**単位行列**にしています。第 `t` 軸は「その語が `t` 番目の語かどうか」だけを
表し、それ以外の意味を持ちません。こうすると `x_t W` は「`W` の第 `t` 行を取り出す」ことと
同じになるため、`W^Q` / `W^K` / `W^V` がそのまま**語ごとの query / key / value の一覧表**として
読めます。軸に「人」「動作」のような人工的な意味を割り当てずに済むのが利点です。
実際のモデルの埋め込み表引きも、内部的にはこれと同じ演算です。

`I`, `like`, `playing`, `football` の4語それぞれについて `z_i` が1本ずつでき、
`softmax(QKᵀ/√d_k)` は 4×4 の行列になります。その第 `i` 行が語 `i` の重み配分です。

既定の設定（`√d_k` で割る・マスクなし）では、attention 行列は次のようになります。

```text
            I     like  playing football
I         0.125  0.730   0.125   0.021   → like      主語 → 述語
like      0.021  0.125   0.730   0.125   → playing   動詞 → 目的語
playing   0.125  0.021   0.125   0.730   → football  動詞 → 目的語
football  0.029  0.078   0.651   0.242   → playing   目的語 → 述語
```
...
- `√d_k` で割るかどうかの切り替え（割らないと softmax が鋭くなる。最大重み 0.730 → 0.854）
...
かつ「主語は述語を探す」「動詞は目的語を探す」という参照関係が現れるように手で書いた値です。
本物のモデルはこの配線をデータから学習します。

`q` / `k` / `v` の各軸には名前を付けず「軸1」「軸2」と番号でだけ呼んでいます。実際のモデルでも
これらの軸に人間が読める意味はなく、意味を持つのは内積の大小と、混ざった結果のベクトルだからです。

「文脈が反映された」ことの確認には**因果マスクのトグル**を使います。`playing` の配分は
マスクなしで `[0.125, 0.021, 0.125, 0.730]`、マスクありで `[0.461, 0.079, 0.461, 0]` となり、
`z_playing` も別のベクトルになります。語は同じでも参照できる文脈が変われば `z` は変わる、
ということが1クリックで確かめられます。

トークン化は簡略化しています。実際のトークナイザは `I like playing football.` を
5トークン以上に分割しますが（句点も1トークン、語によってはサブワードに割れる）、
このページは self-attention の計算だけに集中するため4語として扱っています。

positional encoding は足していません。4語は埋め込みが互いに違うので、足さなくても区別は
つきます。足していない影響は、`football playing like I` と並べ替えても各語の `z` がまったく
同じになることです（attention は置換同変）。語順を扱うには位置符号が要る、というのは
この意味であり、ページ側でもそう説明しています。
